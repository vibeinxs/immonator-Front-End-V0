      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        <Section title={t("analyse.section.property")}>
          <div className="space-y-2">
            <Label htmlFor="analysis-address" className={LABEL}>{t("analyse.field.address")}</Label>
            <Input
              id="analysis-address"
              value={value.address}
          ...
          <div className="space-y-2">
            ...
            >
              {(["existing", "newbuild"] as const).map((condition) => (
                <Label
                  key={condition}
                  htmlFor={`analysis-condition-${condition}`}
                ...
                >
                  <RadioGroupItem id={`analysis-condition-${condition}`} value={condition} />
        ...
        <Section title={t("analyse.section.financing")}>
          <Row label={t("analyse.field.purchasePrice")}>
            <NumberWithUnit id="analysis-purchase-price" value={value.purchase_price} onChange={(nextValue) => setField("purchase_price", nextValue)} unit="€" min={0} />
          </Row>
          <Row label={t("analyse.field.equity")}>
            <NumberWithUnit id="analysis-equity" value={value.equity} onChange={(nextValue) => setField("equity", nextValue)} unit="€" min={0} />
          </Row>
          <Row label={t("analyse.field.interest")}>
            <NumberWithUnit id="analysis-interest" value={value.interest_rate ?? 3.8} onChange={(nextValue) => setField("interest_rate", nextValue)} unit="%" step={0.1} min={0} />
          </Row>
          <Row label={t("analyse.field.repayment")}>
            <NumberWithUnit id="analysis-repayment" value={value.repayment_rate ?? 2} onChange={(nextValue) => setField("repayment_rate", nextValue)} unit="%" step={0.1} min={0} />
          </Row>
          <Row label={t("analyse.field.transferTax")}>
            <NumberWithUnit id="analysis-transfer-tax" value={value.transfer_tax_pct ?? 6} onChange={(nextValue) => setField("transfer_tax_pct", nextValue)} unit="%" step={0.1} min={0} />
          </Row>
          <Row label={t("analyse.field.notary")}>
            <NumberWithUnit id="analysis-notary" value={value.notary_pct ?? 2} onChange={(nextValue) => setField("notary_pct", nextValue)} unit="%" step={0.1} min={0} />
          </Row>
          <Row label={t("analyse.field.agent")}>
            <NumberWithUnit id="analysis-agent" value={value.agent_pct ?? 0} onChange={(nextValue) => setField("agent_pct", nextValue)} unit="%" step={0.1} min={0} />
          </Row>
          <Row label={t("analyse.field.landShare")}>
            <NumberWithUnit id="analysis-land-share" value={value.land_share_pct ?? 20} onChange={(nextValue) => setField("land_share_pct", nextValue)} unit="%" step={1} min={0} max={100} />
          </Row>
        </Section>

        <Section title={t("analyse.section.incomeCosts")}>
          <Row label={t("analyse.field.rent")}>
            <NumberWithUnit id="analysis-rent" value={value.rent_monthly} onChange={(nextValue) => setField("rent_monthly", nextValue)} unit="€/mo" min={0} />
          </Row>
          <Row label={t("analyse.field.hausgeld")}>
            <NumberWithUnit id="analysis-hausgeld" value={value.hausgeld_monthly ?? 200} onChange={(nextValue) => setField("hausgeld_monthly", nextValue)} unit="€/mo" min={0} />
          </Row>
          <Row label={t("analyse.field.maintenance")}>
            <NumberWithUnit id="analysis-maintenance" value={value.maintenance_nd ?? 1200} onChange={(nextValue) => setField("maintenance_nd", nextValue)} unit="€/yr" min={0} />
          </Row>
          <Row label={t("analyse.field.management")}>
            <NumberWithUnit id="analysis-management" value={value.management_nd ?? 600} onChange={(nextValue) => setField("management_nd", nextValue)} unit="€/yr" min={0} />
          </Row>
          <Row label={t("analyse.field.propertyTax")}>
            <NumberWithUnit id="analysis-property-tax" value={value.grundsteuer_annual ?? 0} onChange={(nextValue) => setField("grundsteuer_annual", nextValue)} unit="€/yr" min={0} />
          </Row>
        </Section>

        <Section title={t("analyse.section.assumptions")}>
          <Row label={t("analyse.field.rentGrowth")}>
            <NumberWithUnit id="analysis-rent-growth" value={value.rent_growth ?? 2} onChange={(nextValue) => setField("rent_growth", nextValue)} unit="%" step={0.1} />
          </Row>
          <Row label={t("analyse.field.appreciation")}>
            <NumberWithUnit id="analysis-appreciation" value={value.appreciation ?? 2} onChange={(nextValue) => setField("appreciation", nextValue)} unit="%" step={0.1} />
          </Row>
          <Row label={t("analyse.field.taxRate")}>
            <NumberWithUnit id="analysis-tax-rate" value={value.tax_rate ?? 42} onChange={(nextValue) => setField("tax_rate", nextValue)} unit="%" step={1} min={0} max={100} />
          </Row>
          <Row label={t("analyse.field.horizon")}>
            <NumberWithUnit id="analysis-holding-years" value={value.holding_years ?? 10} onChange={(nextValue) => setField("holding_years", Math.round(nextValue))} unit={t("analyse.unit.yearShort")} step={1} min={1} />
          </Row>
        </Section>
          ...
          <div className="space-y-2">
            ...
            >
              {VACANCY_OPTIONS.map((vacancyRate) => (
                <Label
                  key={vacancyRate}
                  htmlFor={`analysis-vacancy-${vacancyRate}`}
                ...
                >
                  <RadioGroupItem id={`analysis-vacancy-${vacancyRate}`} value={String(vacancyRate)} />
                  {vacancyRate}%
                </Label>
              ))}
            </RadioGroup>
          ...
          {specialAfaEnabled ? (
            <div className="space-y-3 rounded-xl border border-dashed border-border-default bg-bg-elevated/70 p-3">
              <Row label={t("analyse.field.sonderRate")}>
                <NumberWithUnit id="analysis-special-afa-rate" value={value.special_afa_rate_input ?? 5} onChange={(nextValue) => setField("special_afa_rate_input", nextValue)} unit="%" step={1} min={0} />
              </Row>
              <Row label={t("analyse.field.years")}>
                <NumberWithUnit id="analysis-special-afa-years" value={value.special_afa_years ?? 4} onChange={(nextValue) => setField("special_afa_years", Math.round(nextValue))} unit={t("analyse.unit.yearShort")} step={1} min={1} />
              </Row>
            </div>
          ) : null}
        </Section>
